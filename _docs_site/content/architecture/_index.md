---
title: Architecture
summary: Pipeline architecture, component map, data flow, and concurrency model.
audience: [engineer]
weight: 10
---

This document describes how dBpoweramp Wrapper is structured internally. It is aimed at developers who want to understand, extend, or debug the tool. End users should start with [README](https://github.com/5509346169/wrapper_dbpoweramp/blob/main/README.md) and the [landing page]({{< relref "/" >}}).
...
| `integrity.py` | Post-convert verifier | `VerifyStatus` enum, `VerifyResult` dataclass, `verify_file()` dispatcher |
...
Abstract base class + three concrete implementations. The `ConversionBackend` ABC defines `convert()`, `copy()`, `verify()`, `supports()`, and `validate_environment()`. The `validate_environment()` call is made fail-fast at startup (before any file is touched): missing binaries or Wine prefix raise `BackendError` immediately.
