...
{{< callout type="warning" title="BackendError: ffmpeg binary 'ffmpeg' not found on PATH" >}}Linux: install via your package manager. Windows: download from <https://ffmpeg.org/download.html> or use Chocolatey (`choco install ffmpeg`).{{< /callout >}}

{{< callout type="warning" title="BackendError: 'wine' not found on PATH" >}}Linux: install via your package manager (`pacman -S wine`, `apt install wine`, `dnf install wine`).{{< /callout >}}

{{< callout type="warning" title="BackendError: CoreConverter not found" >}}Verify dBpoweramp is installed and that the path in `settings.yaml` matches the install location. On Linux, confirm the Wine prefix exists.{{< /callout >}}

{{< callout type="warning" title="Encoder 'libfdk_aac' is not available" >}}Install an FFmpeg build with FDK AAC. On Arch: `yay -S ffmpeg-full`. On Windows: download a build from <https://www.gyan.dev/ffmpeg/builds/>.{{< /callout >}}

{{< callout type="warning" title="BackendError: WINEPREFIX does not exist" >}}Initialize the prefix with `wineboot --init`, then reinstall dBpoweramp under that prefix.{{< /callout >}}

See [Error handling]({{< relref "engineering/error-handling" >}}) for additional runtime errors.
...
See [Configuration reference]({{< relref "configuration" >}}) for the full schema.
...
{{< callout type="note" title="Wine in Docker" >}}Requires additional configuration and may have limited functionality.{{< /callout >}}

## Next steps

- [CLI reference]({{< relref "configuration/cli" >}}) for usage examples
- [Configuration reference]({{< relref "configuration" >}}) for customization options
- [Workflow]({{< relref "architecture/workflow" >}}) to understand how conversions run
