...
{% include components/callout.html type="warning" title="FDK AAC availability" content="The `requires_encoder` field tells the backend to check `ffmpeg -encoders` before running. If FDK AAC is not available, a clear error message is shown with installation instructions." %}
