...
For `convert` jobs: `run_job` calls `_verify_output_file(job)` (full-frame decode). For `copy` jobs: existence + non-zero size. For `skip` jobs: not called.

A `VERIFY_RESULT` event is enqueued before `FINISHED` for both `--execution-mode hybrid` and `--execution-mode phased`.

Outcomes:

- `NOT_OK` → flipped to `FAILED`, `error_msg = "Not - <reason>"`
- `UNSUPPORTED` → SUCCESS with soft warning logged as `verify skipped: ...`
- `OK` → SUCCESS with no annotation
