...
This document provides detailed reference for each module in the `src/` package. For the architectural role of each module and how they interact at runtime, see [Architecture](/architecture/).
...
```python
def _verify_soundfile(path) -> VerifyResult:
    """Full-frame block decode via libsndfile, FLAC embedded MD5 verified on close, truncation guard."""

def _verify_miniaudio(path) -> VerifyResult:
    """Streaming decode via miniaudio.stream_file(); raises on sync/frame errors."""

def _verify_mutagen(path) -> VerifyResult:
    """easy=False tag/metadata sanity only (last-resort fallback)."""
```

### Constants

```python
UNAMBIGUOUS_LOSSLESS_EXT = frozenset({".flac", ".fla", ".ape", ".wv", ".tta", ".tak",
                                       ".ofr", ".ofs", ".shn",
                                       ".wav", ".aiff", ".aif", ".caf", ".bwf", ".au", ".pcm", ".raw"})

AMBIGUOUS_EXT = frozenset({".m4a", ".mp4", ".caf"})

UNAMBIGUOUS_LOSSY_EXT = frozenset({".mp3", ".mp2", ".mp1",
                                   ".ogg", ".opus", ".spx",
                                   ".wma", ".wmv", ".asf",
                                   ".ac3", ".eac3",
                                   ".dts", ".dtshd", ".dtsma",
...
LOSSY_FOLDER_TOKENS = frozenset({"aac", "mp3", "v0", "v2",
                                 "128k", "192k", "256k", "320k",
...
LOSSLESS_CODECS = frozenset({"flac", "alac", "ape", "wavpack", "tta", "mlp", "truehd",
...
def build_jobs(...) -> tuple[list[ConversionJob], list[Path]]:
...
Single-job execution — copy / convert / skip branches. `_verify_output_file` calls `src.audio.integrity.verify_file()` for `convert` jobs (full-frame decode), falls back to existence + non-zero-size for `copy` jobs, and is not called for `skip` jobs. On `NOT_OK` the job is flipped to `FAILED` with `error_msg = "Not - <reason>"`. A `VERIFY_RESULT` event is enqueued before `FINISHED`.
...
class ConversionDB:
    ...
    def log_conversion(
        ...
        """Insert or update a history row with the current UTC timestamp. The four verify_* kwargs are optional; omitting them writes NULL."""
