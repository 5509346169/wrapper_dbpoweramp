...
See [Lossy source handling](/lossy-handling/) for the full detection cascade.
...
See [Sidecar files](/sidecar-files/) for the full policy reference.
...
For the deep dive see [Architecture](/architecture/).
...
See [Presets](/presets/) for full per-backend arguments.
