...
{% include components/callout.html type="audiophile" title="Why verify?" content="Some tools may exit 0 even when the output is corrupt or truncated. Verification ensures corrupt outputs are never marked as successful." %}
...
See [Workflow §7a](/workflow/#pre-verify-skip-gate) for the demotion policy.
...
{% include components/callout.html type="warning" title="Partial success" content="The wrapper may exit 0 even if some jobs failed. Always check the summary output." %}
...
See [File index system](/file-index/) for the full schema and query examples.
